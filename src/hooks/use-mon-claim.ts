...
async function logClaim(
  ...
  console.log('🚀 SENDING MON CLAIM TO LOG API:', {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('❌ Failed to log MON claim:', response.statusText, errorText)
    } else {
      const result = await response.json()
      console.log('✅ MON claim logged successfully:', result)
    }
  } catch (error) {
    console.error('❌ Failed to log MON claim:', error)
